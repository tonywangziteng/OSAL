...
class DownSample(nn.Module):
    ...
    def __init__(self, config):
        ...
        for idx, out_dim in enumerate(out_dims):
            ...
            self.add_module(
                'down_skip_{}'.format(idx), 
                nn.Conv1d(in_dim, out_dim, kernel_size=1, stride=2)
            )
            in_dim = out_dim
    ...
    def forward(self, feature):
        ...
        for layer_idx in range(5):
            ...
            down_skip_submodule = getattr(self, 'down_skip_{}'.format(layer_idx))
            x_conv = downsample_submodule(x)
            x_skip = down_skip_submodule(x)
            x = torch.relu(x_conv + x_skip)
