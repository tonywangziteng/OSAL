...
def get_gt_dict_2(gt_path, action_name_path):
    ...
    print("length of action list: ", len(context))
    action_name_list = [[context[i], i-1] for i in range(1, len(context))]
    action_name_dict = dict(action_name_list)
    # print(action_name_dict)
    file.close()

    # load ground truth
    # gt = {"video_name": {"duration_second", "duration_frame",
    #                      "annotations":[{"segment":[start_sec, end_sec], "label": }, {"segment", "label"}, ...], }
    #       "video_name_2" ...}
    # 
    ...
    for video_name in f:
        ...
        anno_list = []
        for anno_each in annotations:
            time_sec = anno_each["segment"]
...
def voc_eval(classidx,
    ...
    # gt_dict = [{ 'vid_name': [[start, end, class_index], [start, end, class_index], ...] }, ...]
